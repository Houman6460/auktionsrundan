...
export default function ShareButtons({ title, url, text, image, mapUrl, children }) {
  ...
  const links = {
    // Facebook: prioritize showing the quote text by sharing our page URL, but include the map link inside the quote.
    facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}&quote=${encodeURIComponent(shareText + (mapUrl ? '\n' + mapUrl : ''))}`,
  ...
  return (
    <div className="flex items-center gap-2 w-full">
      <div className="flex flex-wrap items-center gap-1 sm:gap-2 flex-1 min-w-0">
        <button type="button" className="btn-outline text-xs px-2 py-1 h-8" onClick={doWebShare} aria-label="System share" title="System share">
          {/* Share icon */}
        ...
        <button type="button" className="btn-outline text-xs px-2 py-1 h-8" onClick={shareFacebook} aria-label="Dela på Facebook" title="Facebook">
          {/* Facebook icon */}
        ...
        <a className="btn-outline text-xs px-2 py-1 h-8" href={links.twitter} target="_blank" rel="noopener noreferrer" aria-label="Dela på X (Twitter)" title="X (Twitter)">
          {/* X icon */}
        ...
        <a className="btn-outline text-xs px-2 py-1 h-8" href={links.linkedin} target="_blank" rel="noopener noreferrer" aria-label="Dela på LinkedIn" title="LinkedIn">
          {/* LinkedIn icon */}
        ...
        <a className="btn-outline text-xs px-2 py-1 h-8" href={links.whatsapp} target="_blank" rel="noopener noreferrer" aria-label="Dela på WhatsApp" title="WhatsApp">
          {/* WhatsApp icon */}
        ...
        <a className="btn-outline text-xs px-2 py-1 h-8" href={links.telegram} target="_blank" rel="noopener noreferrer" aria-label="Dela på Telegram" title="Telegram">
          {/* Telegram icon */}
        ...
        <button type="button" className="btn-outline text-xs px-2 py-1 h-8" onClick={shareInstagram} aria-label="Dela via Instagram Direct" title="Instagram Direct">
          {/* Instagram icon */}
        ...
        <a className="btn-outline text-xs px-2 py-1 h-8" href={links.sms} aria-label="Dela via SMS" title="SMS">
          {/* SMS icon */}
        ...
        {mapUrl && (
          <a className="btn-outline text-xs px-2 py-1 h-8" href={mapUrl} target="_blank" rel="noopener noreferrer" aria-label="Öppna karta" title="Karta/Directions">
            {/* Map/Pin icon */}
        ...
        <a className="btn-outline text-xs px-2 py-1 h-8" href={links.mail} aria-label="Dela via e-post" title="E‑post">
          {/* Mail icon */}
        ...
        <button type="button" className="btn-outline text-xs px-2 py-1 h-8" onClick={() => copyToClipboard(`${shareText}\n${shareUrl}`)} aria-label="Kopiera" title="Kopiera">
          {/* Copy icon */}
      ...
      {children ? <div className="shrink-0 ml-2">{children}</div> : null}
    </div>
  )
}
